...
def end_game_handler(bot, update, args):  # /endgame
    if not (update.effective_user["id"] in admins):
        ...
        if parameter != "":
            if parameter == "sure":
                game_id = database.get_game_id(update.effective_user["id"])
            ...
            if len(args) > 0:
                if args[len(args) - 1].startswith("#"):  # special marker for question number
                    ...
                    database.set_answer(answer,
                                        database.DBVariables.current_number_of_questions)  # need to think about longer answers
                else:
